...
/**
 *
 * @description : this part is for import SHP file
 */
...
if ($blnLocalFileFlag == 1) {
    $blnUseDbfFile = isset($_POST['nameUseDbfFile']);
    $blnUseShxFile = isset($_POST['nameUseShxFile']);
    $SRSname = $_POST['nameShpSRS'];

    $tempNameShp = $_FILES['nameShpSelect']['tmp_name'];
    $fileNameShp = $_FILES['nameShpSelect']['name'];
    $tempNameDbf = $_FILES['nameDbfSelect']['tmp_name'];
    $fileNameDbf = $_FILES['nameDbfSelect']['name'];
    $tempNameShx = $_FILES['nameShxSelect']['tmp_name'];
    $fileNameShx = $_FILES['nameShxSelect']['name'];

    $file_sizeShp = round(filesize($tempNameShp)/1024);
    ...
    if(($fileNameShp AND !$tempNameShp) OR ($fileNameDbf AND !$tempNameDbf) OR ($fileNameShx AND !$tempNameShx)
	OR $file_sizeShp >=$maxUploadFileSize OR $file_sizeDbf >=$maxUploadFileSize OR $file_sizeShx >=$maxUploadFileSize){
	    $error="You are allowed to upload files with size under 2Mbytes only. if you have big files, please upload the files to data folder in $softName and then run Remote Files Import.";
	}
}
if ($blnLocalFileFlag == 0) {
    $blnUseDbfFile = isset($_POST['RemotenameUseDbfFile']);
    $blnUseShxFile = isset($_POST['RemotenameUseShxFile']);
    $SRSname = $_POST['nameRemoteShpSRS'];

    $tempNameShp = $_POST['RemoteSelectSHP'];
    $fileNameShp = $_POST['RemoteSelectSHP'];
    $tempNameDbf = $_POST['RemoteSelectDBF'];
    $fileNameDbf = $_POST['RemoteSelectDBF'];
    $tempNameShx = $_POST['RemoteSelectSHX'];
    $fileNameShx = $_POST['RemoteSelectSHX'];
}
// get the file name without postfix
$layerName = substr(basename($fileNameShp),0, strlen(basename($fileNameShp))-4);
//echo $layerName;
if (!$fileNameShp) {
    $error = "No SHP file has been selected!";
}
if (!$fileNameDbf && $blnUseDbfFile) {
    $error = "No DBF file has been selected!";
}
if (!$fileNameShp && $blnUseShxFile) {
...
if ($error == "") {
    ...
    $fileNameShp = $tempNameShp;
    $fileNameDbf = $tempNameDbf;
    $fileNameShx = $tempNameShx;

    $shp2db = new SHP2DB($servername, $username, $password, $dbname, $tbname, $layerName, $SRSname,
        $blnUseDbfFile, $blnUseShxFile, $fileNameShp, $fileNameDbf, $fileNameShx);
    // start import now
    //$shp2db->loadFromFile();

    $error = $shp2db->getError();
    $log = $shp2db->getLog();
    $recordgood = $shp2db->recordgood;
    $recordbad = $shp2db->recordbad;
}
