...
if ($database->databaseConnection and $error == "") {
    ...
    while ($line2 = $database->getColumns($rs4)) {
        if ($num == 1) {
            echo "!document.Fgetfeatureinfo.SRS.checked ";
        }
        if ($num > 1) {
            echo "!document.Fgetfeatureinfo.SRS[" . $iSRSs . "].checked ";
        }
 ...
 <table class="tableContent">
        <tr>
          <td width="100%"><h2>GetFeatureInfo Request</h2>&nbsp;</br>Select from the currently loaded SRSs</td>
		</tr>

        <tr>
		<td>
           <table class="tableBlock">
             <form name="Fgetfeatureinfo" action="wms_GetFeatureInfo_r.php" method="get" onSubmit="return chkform()" >
...
<?php
    ...
    while ($line4 = $database->getColumns($rs4)) {
        ...
        ?>	    <tr>
			         <td><input type="radio" name= "SRS"  <?php echo "  value=\"" . $data4 . "\"> "?><?=$data4?></td>
				    </tr>
			<?php
    }
    ...
    if ($data4) {
        ...
        ?>
					<tr>
				 	 <td align="right"><input type="submit" name="SQuery" value="GetFeatureInfo" onmouseover="this.className='button1'" onmouseout="this.className='button'" class="button"></td>
					</tr>
			<?php
    }
