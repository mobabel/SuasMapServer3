...
function writeDivStyleDialog (sLayerName) {
	...
	sContents = '<table cellspacing =0 cellpading =0 border=0>' +
				...
				'<tr><td  colspan=2><img src="../img/range1.jpg" border="0" name="range1" onClick="getRange (this);" style="CURSOR: pointer">' +
				'<img src="../img/range2.jpg" border="0" name="range2"  onClick="getRange (this);" style="CURSOR: pointer">' +
				'<img src="../img/range3.jpg" border="0" name="range3"  onClick="getRange (this);" style="CURSOR: pointer">' +
				'<img src="../img/range4.jpg" border="0" name="range4" onClick="getRange (this);" style="CURSOR: pointer">' +
				'<img src="../img/range5.jpg" border="0" name="range5" onClick="getRange (this);" style="CURSOR: pointer">' +
				'<img src="../img/range6.jpg" border="0" name="range6" onClick="getRange (this);" style="CURSOR: pointer">' +
				'<img src="../img/range7.jpg" border="0" name="range7" onClick="getRange (this);" style="CURSOR: pointer">' +
				'<img src="../img/range8.jpg" border="0" name="range8" onClick="getRange (this);" style="CURSOR: pointer"></td></tr>';
	...
	if ( sLayerTypeUpper == 'POLYGON' ) {
		...
		sContents = sContents + document.frmStyleAction[sLayerName + 'txtFill'].value +  '" size="6" onChange ="handleOnChange (this);" onFocus="handleOnFocus (this);" MAXLENGTH =7></td></tr>';
		...
		sContents = sContents + document.frmStyleAction[sLayerName + 'txtStroke'].value + '" size="6" onChange ="handleOnChange (this);" onFocus="handleOnFocus (this);" MAXLENGTH =7></td></tr>';
		sContents = sContents + '<tr><td>Width:</td><td><input type ="text" name="txtStrokeWidth" class="smallInput" value ="' + document.frmStyleAction[sLayerName + 'txtStrokeWidth'].value
		sContents = sContents + '" size="6" onKeyPress="return handleKeyPress (event);" onKeyUp ="handleOnChange (this);"  onFocus="cancelOnFocus (this);" MAXLENGTH =20></td></tr>';
...
function showLoadingSld(){
	$('INFOMESSAGE').update('<img src=\"../img/wait.gif\" /> Loading Style......');
	//$('INFOMESSAGE').show();
...
function changeStyle(stid){
	...
	var myAjax = new Ajax.Request(
                "getsld.php?"+pars,
                {
					...
					onLoading: showLoadingSld(),
					onComplete: refreshsldcontainer
				}
                );
 ...
 * @return void
 **/
function refreshsldcontainer(originalRequest){
	hideLoadingSld();
	try{
	...
	if(content.indexOf('error') < 0){
		$('sld_styleinfo_containers').update(content);
		//reset the current status
		$('sltLayers').options[0].selected = true;
		writeDivStyleDialog($('sltLayers').options[0].value);
	}else{
...
function saveStyle(){
	...
	if($('sltStylelist').value == 0){
		...
		if(stylename.trim().isEmpty() || ( stylename == 'undefined') || ( stylename == null) || stylename == '' ){
			showErrorMessage("Style name can not be empty!");
	    	return false;
		}
		else{
		...
		document.frmStyleAction.sld_style_id.value = $('sltStylelist').value;
		document.frmStyleAction.action = 'atlas.php';
		document.frmStyleAction.submit();
	}
}
...
function saveAsStyle(){
	...
	if(stylename.trim().isEmpty() || ( stylename == 'undefined') || ( stylename == null) || stylename == '' ){
		...
		document.frmStyleAction.altas_operation.value = 'saveas_sld';
		document.frmStyleAction.sld_style_name.value = stylename;
		document.frmStyleAction.action = 'atlas.php';
		document.frmStyleAction.submit();
	}
...
function ChangeStyleName(){
	...
	if(stylename.trim().isEmpty() || ( stylename == 'undefined') || ( stylename == null) || stylename == '' ){
		...
		document.frmStyleAction.altas_operation.value = 'changename_sld';
		document.frmStyleAction.sld_style_name.value = stylename;
		document.frmStyleAction.sld_style_id.value = $('sltStylelist').value;
		document.frmStyleAction.action = 'atlas.php';
		document.frmStyleAction.submit();
...
function setDefaultStyle(){
	if($('sld_style_default_stid').value == 0){
		showErrorMessage("There is no style!");
	    return false;
	}else{
		//if the current select style is already default style
		if($('sltStylelist').value == $('sld_style_default_stid').value){
			showErrorMessage("This style is already default style.");
	    	return false;
		}else{
			document.frmStyleAction.altas_operation.value = 'setdefault_sld';
			document.frmStyleAction.sld_style_id.value = $('sltStylelist').value;
			document.frmStyleAction.action = 'atlas.php';
			document.frmStyleAction.submit();
		}
...
function deleteStyle(){
	if($('sld_style_default_stid').value == 0){
		...
		//if the current select style is already default style
		if($('sltStylelist').value == $('sld_style_default_stid').value){
			showErrorMessage("Can not delete default style!");
	    	return false;
		}else{
			if(confirm("Are you sure to delete the layers "+$('sltStylelist').options[$('sltStylelist').selectedIndex].text+" ?")){
				document.frmStyleAction.altas_operation.value = 'delete_sld';
				document.frmStyleAction.sld_style_id.value = $('sltStylelist').value;
				document.frmStyleAction.action = 'atlas.php';
				document.frmStyleAction.submit();
			}else{
