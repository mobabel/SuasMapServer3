...
class SHP2DB {
    ...
    public function SHP2DB($servername_, $username_, $password_, $dbname_, $tbname_, $layerName_, $SRSname_ ,
    ...
    function openSHPFile($toWrite = false)
    {
        if (file_exists($this->fileNameShp)) {
            ...
            if (!$this->SHPFile) {
		     ...
		     return $this->setError(sprintf("The Shape file '%s' does not exist", $this->SHPFileName));
		}
        return true;
    }
    ...
    function openSHXFile($toWrite = false)
    {
        if ($this->blnUseShxFile) {
            if (file_exists($this->fileNameShp)) {
                ...
                if (!$this->SHXFile) {
                    return $this->setError(sprintf("It wasn't possible to open the SHX file '%s'", $this->fileNameShx));
                }
                return true;
            }
            else{
                return $this->setError(sprintf("The SHX file '%s' does not exist", $this->fileNameShx));
			}
        }
    }
        ...
        if ($this->blnUseDbfFile) {
            ...
            if (($toWrite) && (!file_exists($this->fileNameDbf))) {
                if (!@dbase_create($this->fileNameDbf, $this->DBFHeader)) {
                    return $this->setError(sprintf("It is not possible to create the DBase file '%s'", $this->fileNameDbf));
                }
            }
            if ($checkFunction($this->fileNameDbf)) {
                $this->DBFFile = dbase_open($this->fileNameDbf, ($toWrite ? 2 : 0));
                if (!$this->DBFFile) {
                    return $this->setError(sprintf("It is not possible to open the DBase file '%s'", $this->fileNameDbf));
                }
            } else {
                return $this->setError(sprintf("It is not possible to find the DBase file '%s'", $this->fileNameDbf));
            }
            return true;
        }
    }
...
class ShapeRecord {
    ...
    public $database, $tbname, $layerName, $SRSname;
    public $error,$log;
    public $recordgood, $recordbad;
    public $attributes = "";

    function ShapeRecord($shapeType_)
    {
    ...
    function loadMultiPointRecord()
    {
        ...
        $this->database->databaseInsertGeometry($this->tbname, $this->layerName, $this->layerName . "_" . $this->recordNumber, GeometryTypePoint,
            $this->SHPData["xmin"], $this->SHPData["ymin"], $this->SHPData["xmax"], $this->SHPData["ymax"], $resultarray[4], "", $this->SRSname, $this->attributes, StyleDefault);
        if ($this->database->databaseGetErrorMessage() != "") {
            $this->error = $this->database->databaseGetErrorMessage();
        }
    }
