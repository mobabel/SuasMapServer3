...
class SendExceptionClass {
        ...
        $minx, $miny, $maxx, $maxy, $width, $height, $enablestretchmap, $defaultwidth, $defaultheight)
    {
        ...
        if (compareStringInsensitive($this->format, "image/pdf") OR compareStringInsensitive($this->format, "image/ezpdf")
                OR compareStringInsensitive($this->format, "image/swf"))
            $this->exceptions = "application/vnd.ogc.se_xml";
    }

    public function sendexception ($errornumber, $errorexceptionstring)
    {
        if (compareStringInsensitive($this->exceptions, "application/vnd.ogc.se_xml")) {
            $this->printXMLException($errornumber, $errorexceptionstring);
        }

        if (compareStringInsensitive($this->exceptions, "application/vnd.ogc.se_inimage")) {
            ...
            if (!compareStringInsensitive($this->format, "image/svg+xml") AND !compareStringInsensitive($this->format, "image/svgt+xml") AND !compareStringInsensitive($this->format, "image/svgb+xml")
            AND !compareStringInsensitive($this->format, "image/svgz+xml") AND !compareStringInsensitive($this->format, "image/svgtz+xml") AND !compareStringInsensitive($this->format, "image/svgbz+xml")
			AND !compareStringInsensitive($this->format, "image/png") AND !compareStringInsensitive($this->format, "image/jpeg")
                    AND !compareStringInsensitive($this->format, "image/gif")AND !compareStringInsensitive($this->format, "image/pdf") AND !compareStringInsensitive($this->format, "image/ezpdf") AND !compareStringInsensitive($this->format, "image/wbmp") AND !compareStringInsensitive($this->format, "image/swf")
                    AND !compareStringInsensitive($this->format, "image/bmp") AND $this->format != "")
                $this->printXMLException($errornumber, $errorexceptionstring);

            if (compareStringInsensitive($this->format, "image/png") OR compareStringInsensitive($this->format, "image/gif") OR compareStringInsensitive($this->format, "image/jpeg")
                    OR compareStringInsensitive($this->format, "image/wbmp") OR compareStringInsensitive($this->format, "image/bmp"))
                $this->printRasterImageException($errornumber, $errorexceptionstring);

            if (compareStringInsensitive($this->format, "image/svg+xml") OR compareStringInsensitive($this->format, "image/svgz+xml")) {
                $this->printSVGException($errornumber, $errorexceptionstring);
            }
            if (compareStringInsensitive($this->format, "image/svgt+xml") OR compareStringInsensitive($this->format, "image/svgb+xml")
			OR compareStringInsensitive($this->format, "image/svgtz+xml") OR compareStringInsensitive($this->format, "image/svgbz+xml")) {
                $this->printSVGTException($errornumber, $errorexceptionstring);
            }
            if (compareStringInsensitive($this->format, "image/pdf")) {
                $this->printPDFException($errornumber, $errorexceptionstring);
            }
            if (compareStringInsensitive($this->format, "image/ezpdf")) {
                $this->printezPDFException($errornumber, $errorexceptionstring);
            }
            if (compareStringInsensitive($this->format, "image/swf")) {
    ...
    public function printXMLException($errornumber, $errorexceptionstring)
    {
        ...
        print('"' . $this->serverhost . $this->wmsservice . '/exception_1_1_1.dtd">' . "\n");
    ...
    public function printRasterImageException($errornumber, $errorexceptionstring)
    {
        ...
        if (compareStringInsensitive($this->format, "image/png")) {
            header("Content-Type:image/png");
            ImagePNG($newImg);
            ImageDestroy($newImg);
        } elseif (compareStringInsensitive($this->format, "image/gif")) {
            header("Content-Type:image/gif");
            Imagegif($newImg);
            ImageDestroy($newImg);
        } elseif (compareStringInsensitive($this->format, "image/jpeg")) {
            header("Content-Type:image/jpeg");
            Imagejpeg($newImg);
            ImageDestroy($newImg);
        } elseif (compareStringInsensitive($this->format, "image/wbmp")) {
            header("Content-Type:image/wbmp");
            imagewbmp($newImg);
            ImageDestroy($newImg);
        } elseif (compareStringInsensitive($this->format, "image/bmp")) {
...
<svg version="1.1" width="' . $this->width . ' px" height="' . $this->height . ' px" viewBox="0 0 ' . $this->width . ' ' . $this->height . '" preserveAspectRatio="xMidYMid meet" xml:space="preserve" xmlns:phpmywmsvg="http://phpmywms.easywms.com" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" >
