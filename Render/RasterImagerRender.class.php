...
class RasterImageRender {
    ...
    public function setRender($minx, $miny, $maxx, $maxy, $width, $height, $image, $enablestretchmap, $hangle=0,$vangle=0, $distance=0)
    {
        ...
        $this->fontpath = "../fonts/".outputTextCountry."/";
        $this->fonttype = "arial";
        $this->fontext = ".ttf";
    ...
    public function createImage($x, $y, $pointNumber, $imagelink, $alpha, $color) // $alpha 0-100, 0 does nothing
    {
        ...
        if ($dst_w > 1 and $dst_h > 1)
            if ($imagelink != "") {
                ...
                switch ($postfix) {
                    case 'JPG': {
                            $im = @imagecreatefromjpeg($imagelink);
                            if ($im) {
                                ...
                                imagecopymerge($this->image, $im, $dst_x, $dst_y, 0, 0, $src_w, $src_h, $alpha);
                            } else// if can not get image, return the rectange
                                imagerectangle($this->image, $Pixel_x[0], $Pixel_x[0], $Pixel_x[2], $Pixel_y[2], $color);
                        }
                        break;
                    case 'PNG': {
                            $im = @imagecreatefrompng($imagelink);
                            if ($im) {
                                $tempimage = imagecreate($dst_w, $dst_h);
                                $src_w = imagesx($im);
                                $src_h = imagesy($im);
                        ...
                        break;
                    case 'GIF': {
                            $im = @imagecreatefromgif($imagelink);
                            if ($im) {
                                $tempimage = imagecreate($dst_w, $dst_h);
                                $src_w = imagesx($im);
                                $src_h = imagesy($im);
