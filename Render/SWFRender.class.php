...
#						    $movie_shape->scaleTo($radiusx / $radiusy, 1);
    ...
    public function createImage($x, $y, $pointNumber, $imagelink, $alpha, $color) // $alpha 0-100, 0 does nothing
    {
        ...
        $dst_h = $Pixel_y[2] - $Pixel_y[0];
            ...
            switch ($postfix) {
                case 'JPG': {
                        //$this->swf->add(new swfbitmap($imagelink));
                        if ($this->swf->addJpegFromFile($imagelink,$dst_x,$dst_y,$dst_w,$dst_h)) {

                        } else// if can not get image, return the rectange
                            $this->swf->rectangle($dst_x, $dst_y, $dst_w, $dst_h);
                    }
                    break;
                case 'PNG': {

                        if ($this->swf->addPngFromFile($imagelink,$dst_x,$dst_y,$dst_w,$dst_h)) {
...
/*
                        $im = $this->swf->load_image("gif", $imagelink, "");
                        if ($im) {
                            $tempimage = imagecreate($dst_w, $dst_h);
                            $src_w = imagesx($im);
                            $src_h = imagesy($im);
                            imagecopyresampled($tempimage, $im, 0, 0, 0, 0, $dst_w, $dst_h, $src_w, $src_h);

                            $this->swf->fit_image($im, $dst_x, $dst_y, "");
                        } else
                            $this->swf->rectangle($dst_x, $dst_y, $dst_w, $dst_h);
*/
                    }
